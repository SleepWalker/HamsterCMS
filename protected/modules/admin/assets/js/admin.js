...
for (var i = 0; i < menuLinks.length; i++) {
    if (document.getElementById('menu' + i)) {
        menuLinks[i].onmouseover = (function(i) {
            return function() {
                showSubMenu(i);
            };
        })(i);
        menuLinks[i].onmouseout = (function(i) {
            return function() {
                hideSubMenu(i);
            };
        })(i);
 ...
 *
 ******************/
var breadcrumps = document.querySelectorAll('ul.breadcrumps');
 ...
 * Коды
 ******************/
var icodes = document.querySelectorAll('input.icodes');

for (var i = 0; i < icodes.length; i++) {
    icodes[i].onclick = function() {
        this.select();
    };
}
 ...
 * Выпадающий список
 ******************/
var dd_menu = document.querySelectorAll('div.drop_down');

for (var i = 0; i < dd_menu.length; i++) {
    dd_menu[i].getElementsByTagName('div')[0].onclick = (function(i) {
        return function() {
            manageDd(dd_menu[i]);
        };
    })(i);
...
$('.container-tabular').on('click', 'a', function() {
    ...
    var nextIndex = $(this).data('model-count'),
        ...
        replace = function(el, prop, start, end) {
            var $el = $(el);
            if (!$el.is('[' + prop + ']')) return;

            if (!end) end = start;
            var regExp = new RegExp('([\\[_])\\d+([\\]_])', 'g');
            var replacement = $el
                .prop(prop)
                .replace(regExp, '$1' + nextIndex + '$2');
...
if (dnd_wrapper) {
    ...
    var rowHeightMargin = Math.round(
        dnd_wrapper.offsetHeight / dnd_rows.length
    );

    var dndUls = dnd_wrapper.getElementsByTagName('ul');
    var treeList = dndUls.length;
    if (treeList) var lvMargin = dndUls[0].offsetLeft - listOffsetLeft;
}
...
function setDragElement(dnd_block) {
    ...
    dnd_drager.onmousedown = function(e) {
        dragStart(dnd_block, e);
        return false;
    };
...
function dragStart(obj, e) {
    ...
    if (objChildren && objChildren.tagName.toLowerCase() == 'ul') {
        ...
        objChildrenPos = {
            x: objChildrenPos.x - e.pageX - lvMargin,
            y: objChildrenPos.y - e.pageY
        };
    ...
    drag = function(e) {
        ...
        if (new_ind != cur_ind) {
            // перемещаем li по DOM (сортировка, изменение положения по вертикали в иерархии)
            ...
            var rowAfter =
                direction < 1 ? dnd_rows[new_ind - 1] : dnd_rows[new_ind];
            if (rowAfter)
                insertAfter(obj, rowAfter); //вставляем li в начало списка
            else dnd_wrapper.insertBefore(obj, dnd_rows[0]);
            ...
            curCoords.y =
                curCoords.y + rowHeightMargin * amountOfSkippedRows * direction;
        ...
        if (treeList) {
            // Только для многоуровневых списков (сортировка по горизонтали в иерархии)
            // Считаем диапазон возможных уровней
            var lvTop = dnd_rows[cur_ind - 1];
            lvTop =
                lvTop && lvTop.parentNode.id != 'dnd_wrapper'
                    ? lvTop.parentNode.getAttribute('level')
                    : 0;
            var lvBot = dnd_rows[cur_ind + 1];
            lvBot =
                lvBot && lvBot.parentNode.id != 'dnd_wrapper'
                    ? lvBot.parentNode.getAttribute('level')
                    : 0;
            ...
            lvBot *= 1;
            lvTop *= 1;
            curLv *= 1; // конвертируем строчки в циферки
            ...
            if (curLv > cursorLv) {
                // уменьшаем уровень li
                levelDown(obj, curLv, cursorLv);
            } else if (curLv < cursorLv && dnd_rows[cur_ind - 1]) {
                // нужно создать новый уровень (у первого li в списке уровень менять нельзя)
                levelUp(obj, curLv, cursorLv);
            }

            // Делаем подсветку текущего уровня, делая прозрачными все остальные
            if (lastObj)
                //Возможно этот Ul уже удален из DOM, потому проверяем, можно ли менять его class
            ...
            dnd_wrapper.className =
                dnd_wrapper === lastObj ? 'highlight' : 'shadow';
        }
        ...
        if (tChList) {
            // список детей (при переносе li с детьми)
            with (tChList.style) {
                top = e.pageY + objChildrenPos.y + 'px';
                left = e.pageX + objChildrenPos.x + 'px';
            }
    ...
    function levelDown(obj, curLv, cursorLv) {
        while (curLv > cursorLv) {
            ...
            if (!afterUl.childNodes.length)
                afterUl.parentNode.removeChild(afterUl);
    ...
    function levelUp(obj, curLv, cursorLv) {
        while (curLv < cursorLv) {
            ...
            if (childUl && childUl.getAttribute('level') == curLv)
                var newLv = childUl;
            else if (
                (childUl = obj.nextSibling) &&
                childUl.getAttribute('level') == curLv
            )
                // вставляем в существующий ul (находится више li)
                var newLv = childUl;
            else {
                // вставляем в существующий ul (находится ниже li)
                //создаем новый ul
            ...
            if (
                obj.nextSibling &&
                obj.nextSibling.getAttribute('level') == curLv
            )
                // если следующий li существует и в добавок на том же уровне, вставляем наш obj перед ним
                newLv.insertBefore(obj, newLv.firstChild);
            else newLv.appendChild(obj);
...
function dragStop() {
    ...
    if (objChildren) {
        // возвращаем детей на место (если перетаскивался li вместе с детьми)
        // ровняем level детей
        ...
        for (var i = 0; i < objChildrenUls.length; i++) {
            objChildrenUls[i].setAttribute(
                'level',
                objChildrenUls[i].getAttribute('level') * 1 + lvldelta
            );
        }
...
ev.attach(
    ...
    function(args) {
        ...
        if (i == 0) var sindex_new = 0;
        else {
            //Ищем li, у которого возьмем индекс
            var sindexDonor = dnd_rows[i];

            if (dnd_rows[i] === dnd_rows[i].parentNode.firstChild)
                // это первый li в списке(ul) подкатегорий, потому нам нужен индекс li, перед этим ul
            ...
            //            li sindex=11
            do {
                sindexDonor = sindexDonor.previousSibling
                    ? sindexDonor.previousSibling
                    : sindexDonor.parentNode;
        ...
        for (var i = 0; i < dnd_rows.length; i++) {
            ...
            if (sign != 0)
                if (sindex >= smin && sindex <= smax) {
                    // если элемент перемещался
                    // Если индекс входит в промежуток, в котором нужно обновить индексы, редактируем его
        ...
        ajax(location.href, {
            ajax: 1,
            ajaxAction: 'setsindex',
            id: id,
            sindexold: sindex_old,
            sindexnew: sindex_new
        });
    },
    'sort'
);
...
*/
    var obj = obj,
        value = value,
        flEqualityType = flEqualityType || false;
...
function insertAfter(obj, after) {
    if (after.nextSibling)
        after.parentNode.insertBefore(obj, after.nextSibling); // после after нету node, потому вставляем наш элемент в конец родителя
    else after.parentNode.appendChild(obj);
}
...
function ajaxSubmitForm(form) {
    ...
    return $.ajax({
        ...
        data: data,
        // disable any processing from jquery to allow FormData
        processData: false,
        contentType: false,
        cahe: false
    }).always(stopLoad);
 ...
 */
function parseAnswer(form, resp) {
    if (!resp) {
...
$(function() {
    $('body').on('submit', '.js-ajax-form', function(event) {
        ...
        ajaxSubmitForm(form).then(
            function(data) {
                parseAnswer(form, data);
            },
...
function centerBlock(id) {
    ...
    block.style.top =
        Math.round(clientTop + (d.body.offsetHeight - bHeight) / 2) + 'px';
