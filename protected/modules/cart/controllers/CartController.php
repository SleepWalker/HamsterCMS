...
class CartController extends Controller
{
    ...
    protected function callStep()
    {
        ...
        if (!$this->stepParams->valid || $this->stepNum == 4) {
            // Если форма не валидная, рендерим текущий шаг еще раз
            return $this;
        }
    ...
    protected function chooseStep()
    {
        ...
        switch ($this->stepNum) {
            ...
            case 3:
                if ((!isset($this->order['address']) || !isset($this->order['user']))
            ...
            case 2:
                ...
                if (!isset($this->order['order']['type'])
    ...
    protected function step2()
    {
        ...
        if (Yii::app()->user->isGuest) {
            if ($this->order['newUser']) {
                // создаем модель для регистрации
                $userModel = new User('register');
            } else {
                // создаем модель для хранения гостевых заказов
        ...
        try {
            ...
            if (!isset($this->order['summary']['action'])) {
                ...
                if ($orderModel->currency > 1 && $orderModel->currency != 8) {
                    if ($orderModel->currency < 5) {
                        // WM
                        $merchantName = 'WM';
                    } elseif ($orderModel->currency < 8) {
                        // Privat24
    ...
    public function sendOrderSummary()
    {
        ...
        if (!empty($bccEmails)) {
            if (strpos($bccEmails, ',')) {
                $bccEmails = preg_split('/ *, */', $bccEmails);
            } else {
